...
\\begin{table}[h!]
    ...
    \\begin{tabular}{|>{\\columncolor[gray]{0.5}\\color{black}\\bfseries\\raggedright\\arraybackslash}b{20mm}|%
                    >{\\columncolor[RGB]{0,176,240}\\color{black}\\centering\\arraybackslash}b{25mm}|%
                    >{\\columncolor[RGB]{255,192,0}\\color{black}\\centering\\arraybackslash}b{25mm}|%
                    >{\\columncolor[RGB]{112,48,160}\\color{black}\\centering\\arraybackslash}b{25mm}|}
        \\hline
        \\rowcolor{titleblue} \\cellcolor[gray]{0.5} & \\multicolumn{3}{c|}{\\textbf{\\color{white} TIME OF USE - Solar Sun Hours}} \\\\ \\hline
        \\cellcolor[gray]{0.5} & \\textbf{High Demand} & \\textbf{Low Demand} & \\textbf{Annual Blended} \\\\ 
        \\cellcolor[gray]{0.5} & \\textbf{JUN-AUG} & \\textbf{SEP-MAY} & \\textbf{Tariff} \\\\ \\hline
        \\rowcolor[RGB]{255,0,0} \\textbf{Peak} & ${currencyDecimal(tariffRate, 4)} & ${currencyDecimal(tariffRate, 4)} & \\cellcolor[RGB]{112,48,160} \\\\ \\hline
        \\rowcolor[RGB]{255,255,0} \\textbf{Standard} & ${currencyDecimal(tariffRate, 4)} & ${currencyDecimal(tariffRate, 4)} & \\cellcolor[RGB]{112,48,160} ${currencyDecimal(tariffRate, 4)} \\\\ \\hline
        ...
        \\textbf{BLENDED} & ${currencyDecimal(tariffRate, 4)} & ${currencyDecimal(tariffRate, 4)} & \\\\ \\hline
        \\rowcolor{titleblue} \\cellcolor[gray]{0.5} & \\multicolumn{2}{|c|}{\\textbf{\\color{white} kVA Demand Charge}} & \\textbf{\\color{white} ${currencyDecimal(demandRate, 2)}} \\\\ \\hline
    \\end{tabular}
\\end{table}
...
\\begin{table}[h!]
    ...
    \\begin{tabular}{|>{\\raggedright\\arraybackslash}b{40mm}|%
                    >{\\raggedleft\\arraybackslash}b{10mm}|%
                    >{\\raggedright\\arraybackslash}b{75mm}|%
                    >{\\raggedleft\\arraybackslash}b{20mm}|}
        \\hline
        ...
        Project Duration (yr) & 20 & Blended Tariff Used In Evaluation & \\cellcolor[gray]{0.8} ${currencyDecimal(tariffRate, 4)} \\\\ \\hline
        Adjusted Discount Rate & 15.00\\% & Electricity kVA cost Year 1 (ZAR / kVA) & ${currencyDecimal(demandRate, 2)} \\\\ \\hline
        Cost of Capital (LCOE calc) & 9.00\\% & Insurance Cost Year 1 (ZAR) & ${currency(insuranceCost)} \\\\ \\hline
        MIRR - Finance rate & 9.00\\% & Replacement Cost At Year 10 (ZAR) & ${currency(replacementCost)} \\\\ \\hline
        MIRR - re-investment rate & 10.00\\% & \\hspace{1em} 10\\% On Solar Module Cost & ${currency(solarModuleCost)} \\\\ \\hline
...
\\renewcommand{\\arraystretch}{1.0}
    ...
    \\hline
    \\rowcolor{titleblue} \\textbf{\\color{white} TENDER RETURN DATA INPUTS} & \\textbf{\\color{white} STAGE 0} \\\\ \\hline
...
export function financialEstimates(simulation: SimulationData): string {
  ...
  const lsRows = leftCol.map((l, i) => {
    const r = rightCol[i];
    const rightPart = r
      ? `\\textbf{STAGE ${r.stage}} & ${num(r.yield, 2)}`
      : `\\cellcolor[gray]{0.8} & \\cellcolor[gray]{0.8}`;
    return `\\textbf{STAGE ${l.stage}} & ${num(l.yield, 2)} & ${rightPart} \\\\ \\hline`;
  }).join("\n        ");

  // Stage 0 standalone table
  ...
  const stage0Table = `
    \\begin{table}[h!]
        ...
        \\renewcommand{\\arraystretch}{1.0} 
        \\setlength{\\extrarowheight}{2pt}
            \\begin{tabular}{|>` + `{\\columncolor{titleblue}\\color{white}\\bfseries\\raggedright\\arraybackslash}b{45mm}|>` + `{\\raggedleft\\arraybackslash}b{20mm}|}
                \\hline
                \\rowcolor{titleblue} \\color{white} FINANCIAL RETURN OUTPUTS & \\centering\\arraybackslash \\color{white} STAGE 0 \\\\ \\hline
  ...
  // Multi-stage financial table builder
  function makeFinancialTable(stages: number[]) {
    const headerCells = stages.map(s => `\\centering\\arraybackslash \\color{white} STAGE ${s}`).join(" & \n        ");
    const colSpec = stages.map(() => ">" + "{\\raggedleft\\arraybackslash}b{20mm}").join("|");
    ...
    \\begin{table}[h!]
        ...
        \\begin{tabular}{|>` + `{\\columncolor{titleblue}\\color{white}\\bfseries\\raggedright\\arraybackslash}b{45mm}|*4{>` + `{\\raggedleft\\arraybackslash}b{20mm}|}}
            \\hline
            \\rowcolor{titleblue} \\color{white} FINANCIAL RETURN OUTPUTS & 
            ${headerCells} \\\\ 
            \\hline
...
\\begin{table}[h!]
    ...
    \\setlength{\\extrarowheight}{2pt} 
    \\begin{tabular}{|>` + `{\\columncolor{titleblue}\\color{white}\\bfseries\\raggedright\\arraybackslash}b{25mm}|%
                    >` + `{\\raggedleft\\arraybackslash}b{25mm}|%
                    >` + `{\\columncolor{titleblue}\\color{white}\\bfseries\\raggedright\\arraybackslash}b{25mm}|%
                    >` + `{\\raggedleft\\arraybackslash}b{25mm}|}
        \\hline
        \\rowcolor{titleblue} \\color{white} LOAD SHEDDING & \\centering\\arraybackslash \\color{white} KWH / ANNUM & \\color{white} LOAD SHEDDING & \\centering\\arraybackslash \\color{white} KWH / ANNUM \\\\ \\hline
        ${lsRows}
    \\end{tabular}
\\end{table}
