...
export async function generateProfessionalPDF(
  ...
  const addPageFooter = (pageNum: number, totalPages: number) => {
    ...
    // Use company name from branding if available
    const footerText = branding?.company_name || "Generated by SolarSim Pro";
  ...
  if (companyLogoData) {
    try {
      pdf.addImage(companyLogoData, 'PNG', margin, 8, 35, 15);
  ...
  if (clientLogoData) {
    try {
      pdf.addImage(clientLogoData, 'PNG', pageWidth - margin - 35, 8, 35, 15);
    } catch { /* Skip if image fails */ }
  }

  // Project name - shift down if logos present
  const titleY = (companyLogoData || clientLogoData) ? 38 : 28;
  pdf.setFontSize(26);
  pdf.setFont("helvetica", "bold");
  setColor(COLORS.white);
  pdf.text(projectName, margin, titleY);
  
  // Subtitle
  pdf.setFontSize(13);
  pdf.setFont("helvetica", "normal");
  pdf.text("Solar Energy Proposal", margin, titleY + 12);
  
  // Date (positioned based on whether logos exist)
  pdf.setFontSize(9);
  if (!clientLogoData) {
    pdf.text(new Date().toLocaleDateString("en-ZA", { year: "numeric", month: "long", day: "numeric" }), pageWidth - margin - 50, titleY);
  }
