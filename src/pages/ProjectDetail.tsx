...
import { ArrowLeft, Users, BarChart3, DollarSign, Zap, Plug, Sun, CloudSun, FileText, LayoutDashboard, ScrollText, Wallet, CheckCircle2, AlertCircle, Lock, Circle, CalendarIcon, Save, TrendingUp, Leaf, Battery, Building2, MapPin, CalendarDays, FolderOpen, Cpu, ChevronRight, Home } from "lucide-react";
  ...
  const { data: tenants, isLoading: isLoadingTenants } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: tenantsData, error: tenantsError } = await supabase
        ...
        .select(`*, shop_types(*), scada_imports(shop_name, area_sqm, load_profile_weekday, load_profile_weekend, date_range_start, date_range_end, detected_interval_minutes)`)
        .eq("project_id", id)
        .order("name");
    ...
    <div className="space-y-6 min-w-0 max-w-full overflow-x-hidden">
      ...
      <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        {activeTab !== "overview" && (
          <>
            <ChevronRight className="h-3.5 w-3.5" />
            <span className="text-foreground font-medium capitalize">{activeTab.replace(/-/g, ' ')}</span>
          </>
        )}
      </nav>
